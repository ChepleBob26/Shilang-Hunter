...
- Developers: Bob.
- Special Thanks: Tsuki - Give some advice to improve Shilang_Hunter.
...
- Some information by Lang:Uh, ask, I don't have the Chinese winter vacation homework page 1 2,
who has please send it, thank you.
...
if date == "04-01" or date == "05-13":  # 游戏标题和图标初始化
    ...
    game_caption: str = "你施朗了（Code&Release, 开发版, Just For Fun）"
else:
    game_icon: any = pygame.image.load("assets/images/icon.png")
    game_caption: str = "施朗猎人（Code&Release, 开发版）"
...
class Switch:
    ...
    def draw(self, can_active: Union[bool, list]) -> bool:
        ...
        self.rect: Rect = pygame.Rect(self.x, self.y, self.switch_width, self.switch_height)
...
class CustomText:
    ...
    def __init__(self, text: str, font_path: str, font_size: int, font_color: tuple, pos: list, alpha: int,
                 center: int, x_grid: tuple, y_grid: tuple, center2: bool) -> None:
        """初始化自定义文本对象"""
    ...
    def draw(self) -> None:
        ...
        if self.center != 3:  # 居中定位
            if self.center != 2:
                self.pos[0]: int = width // self.x_grid[1] * self.x_grid[0]
            if self.center != 1:
                self.pos[1]: int = height // self.y_grid[1] * self.y_grid[0]
...
class CustomInputBox:
    ...
    def update_rect(self) -> None:
        ...
        if self.center != 3:
            ...
            if self.center != 1:
                self.y: int = height // self.y_grid[1] * self.y_grid[0]
        self.rect: Rect = pygame.Rect(self.x, self.y, new_width, self.height)
        self.rect.center = (self.x, self.y)
        if self.active:
            self.txt_surface: any = self.font.render(self.text, True, self.text_color_active)
        ...
        self.txt_rect: Rect = self.txt_surface.get_rect(center=self.rect.center)
...
class EntityCreate:
    ...
    def __init__(self, game_start_time: int, level: int, stage: int, difficult: int) -> None:
        ...
        for x in range(json_str[f"{level}-{stage}"]["basic"]["enemy"][difficult]):
            self.location.append([json_str[f"{level}-{stage}"][f"entity{difficult}"][f"{x}"]["pos"][0],
                                 json_str[f"{level}-{stage}"][f"entity{difficult}"][f"{x}"]["pos"][1]])
        ...
        if can_active:
            ...
            if not self.progress == json_str[f"{self.level}-{self.stage}"]["basic"]["enemy"][self.difficult]:
                if (self.game_now_time - self.game_start_time - self.total_frozen_time >=
                        ...
                        ["visible_time/ms"]):
                    self.blit[self.progress]: bool = True
                    self.progress += 1
        else:
    ...
    def create(self, can_active: bool, gun_is_shoot: bool, gun_rect: Rect, gun_damage: int, basic_information: list)\
            -> None:
        """生成实体"""
        with open("data/Entity.json") as f:
            json_str: any = json.load(f)
        ...
        for x in range(json_str2[f"{self.level}-{self.stage}"]["basic"]["enemy"][self.difficult]):  # 对每个敌人进行操作
            if self.blit[x]:  # 如果允许输出就开始操作
                ...
                entity_image: any = pygame.transform.scale(entity_image, (
                    ...
                    ["scale"][1]))  # 修改图片尺寸
                if can_active:
                    self.location[x][1] += json_str[json_str2[f"{self.level}-{self.stage}"][f"entity{self.difficult}"]
                                                             [f"{x}"]["id"]]["Speed"]  # 移动
                entity_rect: Rect = pygame.Rect(
                    0, 0,
                ...
                if can_active:
                    ...
                    if (pygame.time.get_ticks() - self.total_frozen_time - self.invincible_time[x] -
                        ...
                        self.can_hit[x]: bool = True
                        self.blink[x]: bool = False
                    self.last_can_hit[x]: bool = self.can_hit[x]
...
class GameGun:
    ...
    def __init__(self, game_start_time: int, gun_type: int, shoot: list, account: int, bullets: Union[str, int]) -> \
            None:
        self.now_time: int = 0
        ...
        self.bullets: Union[str, int] = bullets
        self.damage: int = 1
        self.gun_rect: Rect = (0, 0, 0, 0)
    ...
    def draw(self, can_active: bool) -> None:
        ...
        if can_active:
            ...
            self.gun_rect: Rect = background_image.get_rect(center=mouse_pos)
            ...
            indicator_image_rect: Rect = pygame.Rect(mouse_pos[0], mouse_pos[1], 80, 80)
            ...
            if self.json_str["setting"][self.account]["bullet_visible"]:  # 根据设置调整不同子弹显示方式
                ...
                text_rect: Rect = bullets_text.get_rect()
                ...
                for x in range(self.surplus_bullets):
                    bullet_rect: Rect = pygame.Rect(0, mouse_pos[1] + 50, 30, 30)
        ...
        if can_active:
            ...
            if self.is_accumulation and not pygame.mouse.get_pressed()[0]:  # 结束蓄力
                ...
                if ((pygame.time.get_ticks() - self.accumulation_now_time - self.total_frozen_time -
                    ...
                    self.damage: float = (pygame.time.get_ticks() - self.accumulation_now_time -
                                          self.total_frozen_time - self.game_start_time) / 100
                self.now_time: int = pygame.time.get_ticks() - self.game_start_time - self.total_frozen_time
                self.can_action: bool = False
            ...
            if self.action:  # 开枪状态
                ...
                if timer - self.now_time > self.json_str2[f"{self.gun_type}"]["time/ms"]:
                    self.shoot[0]: bool = False
                    self.damage: float = 1
            self.damage *= self.json_str2[f"{self.gun_type}"]["basic_damage"]
            return
        else:
...
class Pages:
    ...
    def setting_page(self) -> None:
        ...
        text1: any = CustomText("使用文本显示子弹数量", "assets/fonts/Text.TTF", 50,
                                (255, 255, 255), [width // 6, height // 16 * 5], 255, 0, (1, 6), (3, 8), False)
        text2: any = CustomText("数值面板碰到鼠标时透明", "assets/fonts/Text.TTF", 50,
                                (255, 255, 255), [width // 6, height // 16 * 7], 255, 0, (1, 6), (4, 8), False)
        text3: any = CustomText("开启全屏", "assets/fonts/Text.TTF", 50,
                                (255, 255, 255), [width // 6, height // 16 * 9], 255, 0, (1, 6), (5, 8), False)
        while running:
            self.sleep(300)
            screen.fill((200, 200, 200))
            self.bg.update()
            ...
            if button2.draw(50, (180, 180, 180), (255, 255, 255),
                            (180, 180, 180), self.can_click):
                json_str["setting"][self.account]["bullet_visible"]: bool = switch1.activate
                json_str["setting"][self.account]["dock_hidden"]: bool = switch2.activate
                json_str["setting"][self.account]["full_screen"]: bool = switch3.activate
            ...
            if (button1.draw(50, (180, 180, 180), (255, 255, 255),
                ...
                try:
                    ...
                    if json_str["password"][name_index] == input_box2.text:
                        if json_str["setting"][name_index]["full_screen"]:
                            width = full_width
                            height = full_height
                            screen = pygame.display.set_mode((width, height), pygame.FULLSCREEN)
                        else:
                        ...
                        json_str2["log_user"]: int = name_index
    ...
    def account_page(self) -> None:
        ...
        while running:
            ...
            if button1.draw(50, (180, 180, 180), (255, 255, 255),
                ...
                if input_box1.text != "":
                    f: any = open(f"data/User.json")
                    a: any = open(f"data/Level.json")
                    b: any = open(f"data/Gun.json")
    ...
    def game_page(self, level: int) -> None:
        ...
        with open(f"data/Level_data.json") as a:
            json_str2: any = json.load(a)
            a.close()
        create: any = EntityCreate(pygame.time.get_ticks(), level, json_str["level"][self.account], difficult)
        ...
        basic_information: list = [hp, enemies, bullets]
        running: bool = True
        if date == "04-01" or date == "05-13":
            ...
            image3: any = Image(image_usually_use[7], 80, 80, 200, 50, 22, 255, False, False, (2, 8), (1, 2))
            image4: any = Image(image_usually_use[7], 80, 80, 500, 50, 22, 255, False, False, (4, 8), (1, 2))
            image5: any = Image(image_usually_use[7], 80, 80, 800, 50, 22, 255, False, False, (6, 8), (1, 2))
        else:
            ...
            image3: any = Image(f"assets/images/HP.png", 80, 80, 200, 50, 22, 255, False, False, (2, 8), (1, 2))
            image4: any = Image(f"assets/images/Enemies.png", 80, 80, 500, 50, 22, 255, False, False, (4, 8), (1, 2))
            image5: any = Image(f"assets/images/Bullets.png", 80, 80, 800, 50, 22, 255, False, False, (6, 8), (1, 2))
        dock_rect: Rect = pygame.Rect(width // 2, 50, image2.image_width, image2.image_height)
        dock_rect.center = (width // 2, 50)
        gun: any = GameGun(pygame.time.get_ticks(), 0, shoot, self.account, bullets)
        while running:
            ...
            if self.link_up.can_exit:
                ...
                if not can_pause and not pause:
                    can_pause: bool = True
                change_page: bool = True
                self.link_up.access(2000)
            # 更新屏幕
            pygame.display.flip()
            clock.tick(tick_frequency)
